"""
PostgreSQL AsyncPicodb - async PostgreSQL wrapper with dataclass schemas and native FTS.
...
logger = logging.getLogger("picodb.postgres")

from .formatter import data_formatter
from .query import Q
from .postgres_fts import FtsMixinPG
...
class AsyncPicodoPG(Generic[SchemaT], FtsMixinPG):
	"""
	AsyncPicodoPG v4.1 — PostgreSQL wrapper optimized for large datasets.

	Replaces AsyncPicodb for PostgreSQL with native full-text search support.
	"""
	...
	def __init__(
		...
		self._model: Optional[Type[Any]] = None
	...
	def _create_model(self):
		attrs: Dict[str, Any] = {"__tablename__": self._table_name}
		...
		if sqlalchemy_indexes:
			attrs["__table_args__"] = tuple(sqlalchemy_indexes)  # type: ignore

		if self._fts_enabled and self._fts_fields:
			attrs["fts_vector"] = mapped_column(TSVECTOR, nullable=True)

		self._model = type(f"{self.schema_cls.__name__}Model", (Base,), attrs)

	async def init_db(self):
		"""Initialize database schema and FTS support."""
		async with self.engine.begin() as conn:
			...
			if self._fts_enabled and self._fts_fields:
				...
				try:
					await conn.run_sync(self._create_fts_columns)
					await self._create_fts_trigger(conn)
				except Exception as e:
					logger.warning(f"FTS initialization failed: {e}")
			await conn.commit()

	async def _create_fts_trigger(self, conn):
		"""Create trigger to auto-populate FTS columns on insert/update."""
		tsvector_cols = " || ' ' || ".join([f"COALESCE(NEW.{f}, '')" for f in self._fts_fields])
		trigger_sql = f"""
		CREATE OR REPLACE FUNCTION {self._table_name}_fts_update() RETURNS trigger AS $func$
		BEGIN
			NEW.fts_vector := to_tsvector('simple', {tsvector_cols});
			RETURN NEW;
		END;
		...
		drop_sql = f"DROP TRIGGER IF EXISTS {self._table_name}_fts_update_trigger ON {self._table_name};"
		await conn.execute(text(drop_sql))

		create_sql = f"""
		CREATE TRIGGER {self._table_name}_fts_update_trigger
		BEFORE INSERT OR UPDATE ON {self._table_name}
		FOR EACH ROW EXECUTE FUNCTION {self._table_name}_fts_update();
		"""
		await conn.execute(text(create_sql))

	def _create_fts_columns(self, conn):
		"""Add FTS column to table."""
		try:
			from sqlalchemy import text as sql_text
			conn.execute(sql_text(f"ALTER TABLE {self._table_name} ADD COLUMN IF NOT EXISTS fts_vector tsvector;"))
			conn.execute(sql_text(f"CREATE INDEX IF NOT EXISTS {self._table_name}_fts_idx ON {self._table_name} USING gin(fts_vector);"))
		except Exception:
			pass
			...
			async with self.engine.begin() as conn:
				...
				if self._fts_enabled and self._fts_fields:
					try:
						...
						logger.warning(f"FTS trigger recreation failed: {e}")
				await conn.commit()
	...
	async def insert(self, obj: SchemaT) -> str:
		...
		data_dict = asdict(obj)  # type: ignore
		
# Identify JSON fields (list/dict types) to skip data_formatter
		json_fields = {
				...
				for attempt in range(3):
					try:
						...
						if hasattr(obj, "record_id"):
							setattr(obj, "record_id", record_id)
	...
	async def insert_many(self, objects: List[SchemaT], chunk_size: int = 10000, atomic: bool = False) -> List[str]:
		...
		json_fields = {
			f.name for f in fields(self.schema_cls)
			if (t := self._type_hints.get(f.name)) and (
				get_origin(t) in (list, dict, List, Dict) or is_dataclass(t)
			)
		}
		...
		async with self._write_lock:
			async with self.session_factory() as session:
				...
				try:
					...
					await session.rollback()
					logger.error(f"insert_many failed after {chunk_count} chunks: {e}")
					raise
	...
	async def update(self, record_id: str, new_obj: SchemaT):
		...
		data_dict = asdict(new_obj)  # type: ignore
	...
	async def _stream_search(self, predicate, pre_filter, use_fts, fts_query, limit, offset=0):
		...
		if use_fts and self._fts_enabled and fts_query and hasattr(self._model, "fts_vector"):
			async with self.session_factory() as session:
				fts_name = self._table_name
				sql_query = text(
					f"SELECT record_id FROM {fts_name} "
					f"WHERE fts_vector @@ plainto_tsquery('simple', :q) "
					f"LIMIT :lim"
				)
				result = await session.execute(
					sql_query,
					{"q": fts_query, "lim": limit or 100}
				)
				record_ids = [r[0] for r in result.fetchall()]
				if not record_ids:
					return
